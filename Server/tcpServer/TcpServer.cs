            ...
            while (m_isOpen && m_port >= 0)
            {
                try
                {
                    ...
                    for (int i = 0; i < connections.Count; i++)//
                    {
                        ...
                        if (connections[i].CallbackThread != null)//سعی می کند ترد های وصل شده به کانکشن را حذف نمایید
                        {
                            try
                            {
                        ...
                        //اگر تردی روی کانکشن مشغول بود دست به کاری نمی زند
                        if (connections[i].CallbackThread != null) { }
                        //وگرنه چک می کند اگر کانکشن همچنان برقرار بود کانکشن را دوباره برای پردازش ارسال می کند
                        ...
                        {
                            //اگر یکی از کانکشن ها نیاز به کار داشت مورورک را ترو می کند
                            moreWork = moreWork || processConnection(connections[i]);
                        }
                        //وگرنه کانکشن را حذف می کند
                        else
                        {
                            lock (connections)
                            {
                    ...
                    if (!moreWork)
                    {
                        System.Threading.Thread.Yield();//فرصت دادن به دیگر تردها
                        lock (sem)
                        {
                        ...
                        if (!moreWork)//اگر کاری نباشه این بلوک کد در همه تردها به حالت انتظار می روند
                        {
                            waiting = true;
                            sem.Wait(m_idleTime);
                            waiting = false;
