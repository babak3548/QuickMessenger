package ir.bilgisoft.toopeto.xmpp.jid;
...
public final class Jid {
   ...
   //public static final String constantDomainpart="@s.2p2.ir";
    public static final String constantDomainpart="192.168.1.5";
    ...
    public static Jid fromParts(final String localpart)
    {
        try {
            String out;
            if (localpart == null || localpart.isEmpty()) {
                out = constantDomainpart;
            } else {
                out = localpart + "@" + constantDomainpart;
            }
        ...
        {
            Log.d("critic error:", "error in create jid ");
            return  null;
        }
	...
	private Jid(final String jid) throws InvalidJidException {
		...
		if (jid.isEmpty() || jid.length() > 3071) {
			throw new ir.bilgisoft.toopeto.xmpp.jid.InvalidJidException(ir.bilgisoft.toopeto.xmpp.jid.InvalidJidException.INVALID_LENGTH);
		}
		...
		if (jid.startsWith("@") || (jid.endsWith("@") && slashCount == 0) || jid.startsWith("/") || (jid.endsWith("/") && slashCount < 2)) {
			throw new ir.bilgisoft.toopeto.xmpp.jid.InvalidJidException(ir.bilgisoft.toopeto.xmpp.jid.InvalidJidException.INVALID_CHARACTER);
		}
		...
		if (atCount == 0 || (atCount > 0 && slashLoc != -1 && atLoc > slashLoc)) {
			...
			try {
				...
				throw new ir.bilgisoft.toopeto.xmpp.jid.InvalidJidException(ir.bilgisoft.toopeto.xmpp.jid.InvalidJidException.STRINGPREP_FAIL, e);
			}
			if (localpart.isEmpty() || localpart.length() > 1023) {
				throw new ir.bilgisoft.toopeto.xmpp.jid.InvalidJidException(ir.bilgisoft.toopeto.xmpp.jid.InvalidJidException.INVALID_PART_LENGTH);
			}
		...
		if (dp.endsWith(".")) {
			try {
				...
				throw new ir.bilgisoft.toopeto.xmpp.jid.InvalidJidException(e);
			}
		} else {
			try {
	...
	public String toBareJid() {
		try {
			return localpart;// resourcepart.isEmpty() ? this : fromParts(localpart, domainpart, "");
		} catch (final Exception e) {
			// This should never happen.
			return null;
		}
	}
//ghesmat domain bad az @ ra bar mighardanad
