package ir.bilgisoft.toopeto.xmpp.pep;
...
public class Avatar {
	...
	public static Avatar parseMetadata(ir.bilgisoft.toopeto.xml.Element items) {
		ir.bilgisoft.toopeto.xml.Element item = items.findChild("item");
		if (item == null) {
			return null;
		}
		ir.bilgisoft.toopeto.xml.Element metadata = item.findChild("metadata");
		if (metadata == null) {
			return null;
		}
		...
		for (ir.bilgisoft.toopeto.xml.Element child : metadata.getChildren()) {
