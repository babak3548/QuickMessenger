...
//baraye moderiate goftegoy chand nafare ya room
public class MucOptions {
	...
	public class User {
		...
		public void setJid(ir.bilgisoft.toopeto.xmpp.jid.Jid jid) {
			this.jid = jid;
		}
	...
	public void processPacket(UserPacket packet, ir.bilgisoft.toopeto.crypto.PgpEngine pgp) {
		final String from = packet.from;
		if (from == null || from != "") {
			final String name = from;
			final String type = packet.type;
			//final ir.bilgisoft.toopeto.xml.Element x = packet.findChild("x", "http://jabber.org/protocol/muc#user");
		//final List<String> codes = getStatusCodes(x);
			if (type == null) {
				/*	User user = new User();
			if (x != null) {
					ir.bilgisoft.toopeto.xml.Element item = x.findChild("item");
					if (item != null) {
						user.setName(name);
						user.setAffiliation(item.getAttribute("affiliation"));
						user.setRole(item.getAttribute("role"));
						user.setJid(item.getAttributeAsJid("jid"));
						//if (codes.contains(STATUS_CODE_SELF_PRESENCE) || packet.from.equals(this.conversation.getJid())) {
						if ( packet.from.equals(this.conversation.getJid())) {
							this.isOnline = true;
						...
						if (pgp != null) {
							ir.bilgisoft.toopeto.xml.Element signed = packet.findChild("x", "jabber:x:signed");
							if (signed != null) {
		...
		//	} else if (type.equals("unavailable")) {
			} else if (type == Enums.UserTypeEnum.deleteUser.toString()) {
                deleteUser(name);
                /*
			...
			//} else if (type.equals("error")) {
			} else if  (type == Enums.UserTypeEnum.error.toString()){
				if (packet.Description.contains("conflict")) {
					if (isOnline) {
				...
				} else if (packet.Description.contains("not-authorized")) {
					setError(ERROR_PASSWORD_REQUIRED);
				} else if (packet.Description.contains("forbidden")) {
					setError(ERROR_BANNED);
				} else if (packet.Description.contains("registration-required")) {
					setError(ERROR_MEMBERS_ONLY);
				} else {
					setError(ERROR_UNKNOWN);
				}
	...
	private List<String> getStatusCodes(ir.bilgisoft.toopeto.xml.Element x) {
		List<String> codes = new ArrayList<String>();
		if (x != null) {
			for (ir.bilgisoft.toopeto.xml.Element child : x.getChildren()) {
	...
	public ir.bilgisoft.toopeto.xmpp.jid.Jid createJoinJid(String nick) {
		try {
			return ir.bilgisoft.toopeto.xmpp.jid.Jid.fromString(this.conversation.getJid().toBareJid().toString() + "/" + nick);
		} catch (final ir.bilgisoft.toopeto.xmpp.jid.InvalidJidException e) {
			return null;
		}
	}

	public ir.bilgisoft.toopeto.xmpp.jid.Jid getTrueCounterpart(String counterpart) {
