...
public class ScramSha1 extends ir.bilgisoft.toopeto.crypto.sasl.SaslMechanism {
		...
		CACHE = new LruCache<String, KeyPair>(10) {
			protected KeyPair create(final String k) {
				...
				try {
					...
					saltedPassword = hi(ir.bilgisoft.toopeto.utils.CryptoHelper.hexToString(kparts[1]).getBytes(),
							Base64.decode(ir.bilgisoft.toopeto.utils.CryptoHelper.hexToString(kparts[2]), Base64.DEFAULT), Integer.valueOf(kparts[3]));
	...
	public ScramSha1(final ir.bilgisoft.toopeto.xml.TagWriter tagWriter, final ir.bilgisoft.toopeto.entities.Account account, final SecureRandom rng) {
	...
	public String getClientFirstMessage() {
		if (clientFirstMessageBare.isEmpty() && state == State.INITIAL) {
			clientFirstMessageBare = "n=" + ir.bilgisoft.toopeto.utils.CryptoHelper.saslEscape(ir.bilgisoft.toopeto.utils.CryptoHelper.saslPrep(account.getUsername())) +
				",r=" + this.clientNonce;
	...
	public String getResponse(final String challenge) throws AuthenticationException {
		switch (state) {
			case AUTH_TEXT_SENT:
				...
				final ir.bilgisoft.toopeto.crypto.sasl.Tokenizer tokenizer = new ir.bilgisoft.toopeto.crypto.sasl.Tokenizer(serverFirstMessage);
				String nonce = "";
				...
				final KeyPair keys = CACHE.get(
						ir.bilgisoft.toopeto.utils.CryptoHelper.bytesToHex(account.getJid().toBareJid().toString().getBytes()) + ","
						+ ir.bilgisoft.toopeto.utils.CryptoHelper.bytesToHex(account.getPassword().getBytes()) + ","
						+ ir.bilgisoft.toopeto.utils.CryptoHelper.bytesToHex(salt.getBytes()) + ","
	...
	private static synchronized byte[] hi(final byte[] key, final byte[] salt, final int iterations)
		throws InvalidKeyException {
		byte[] u = hmac(key, ir.bilgisoft.toopeto.utils.CryptoHelper.concatenateByteArrays(salt, ir.bilgisoft.toopeto.utils.CryptoHelper.ONE));
		byte[] out = u.clone();
