...
public class AvatarService {
	...
	protected ir.bilgisoft.toopeto.services.XmppConnectionService mXmppConnectionService = null;

	public AvatarService(ir.bilgisoft.toopeto.services.XmppConnectionService service) {
		this.mXmppConnectionService = service;
	}

	public Bitmap get(final ir.bilgisoft.toopeto.entities.Contact contact, final int size) {
	...
	public void clear(ir.bilgisoft.toopeto.entities.Contact contact) {
	...
	private String key(ir.bilgisoft.toopeto.entities.Contact contact, int size) {
		synchronized (this.sizes) {
	...
	public Bitmap get(ir.bilgisoft.toopeto.entities.ListItem item, int size) {
		if (item instanceof ir.bilgisoft.toopeto.entities.Contact) {
			return get((ir.bilgisoft.toopeto.entities.Contact) item, size);
		} else if (item instanceof ir.bilgisoft.toopeto.entities.Bookmark) {
			ir.bilgisoft.toopeto.entities.Bookmark bookmark = (ir.bilgisoft.toopeto.entities.Bookmark) item;
	...
	public Bitmap get(ir.bilgisoft.toopeto.entities.Conversation conversation, int size) {
	...
	public void clear(ir.bilgisoft.toopeto.entities.Conversation conversation) {
		if (conversation.getMode() == ir.bilgisoft.toopeto.entities.Conversation.MODE_SINGLE) {
	...
	public Bitmap get(ir.bilgisoft.toopeto.entities.MucOptions mucOptions, int size) {
		final String KEY = key(mucOptions, size);
		Bitmap bitmap = this.mXmppConnectionService.getBitmapCache().get(KEY);
		if (bitmap != null) {
			return bitmap;
		}
		final List<ir.bilgisoft.toopeto.entities.MucOptions.User> users = new ArrayList<>(mucOptions.getUsers());
		...
		if (count == 0) {
			...
			final int color = ir.bilgisoft.toopeto.utils.UIHelper.getColorForName(name);
	...
	public void clear(ir.bilgisoft.toopeto.entities.MucOptions options) {
		synchronized (this.sizes) {
			for (Integer size : sizes) {
				this.mXmppConnectionService.getBitmapCache().remove(
						key(options, size));
	...
	private String key(ir.bilgisoft.toopeto.entities.MucOptions options, int size) {
		synchronized (this.sizes) {
			if (!this.sizes.contains(size)) {
				this.sizes.add(size);
			}
	...
	public Bitmap get(ir.bilgisoft.toopeto.entities.Account account, int size) {
	...
	public void clear(ir.bilgisoft.toopeto.entities.Account account) {
		synchronized (this.sizes) {
			for (Integer size : sizes) {
				this.mXmppConnectionService.getBitmapCache().remove(
						key(account, size));
	...
	private String key(ir.bilgisoft.toopeto.entities.Account account, int size) {
		synchronized (this.sizes) {
			if (!this.sizes.contains(size)) {
				this.sizes.add(size);
			}
	...
	private void drawTile(Canvas canvas, ir.bilgisoft.toopeto.entities.MucOptions.User user, int left,
						  int top, int right, int bottom) {
		ir.bilgisoft.toopeto.entities.Contact contact = user.getContact();
		if (contact != null) {
			Uri uri = null;
			if (contact.getProfilePhoto() != null) {
