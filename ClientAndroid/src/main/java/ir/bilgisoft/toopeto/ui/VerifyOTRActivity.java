...
import ir.bilgisoft.toopeto.utils.CryptoHelper;
import ir.bilgisoft.toopeto.utils.XmppUri;
...
public class VerifyOTRActivity extends XmppActivity implements XmppConnectionService.OnConversationUpdate {
	...
	private ir.bilgisoft.toopeto.entities.Conversation mConversation;
	private int mode = MODE_MANUAL_VERIFICATION;
	private ir.bilgisoft.toopeto.utils.XmppUri mPendingUri = null;
	...
	private View.OnClickListener mRetrySharedSecretListener = new View.OnClickListener() {
		@Override
		public void onClick(View view) {
			mConversation.smp().status = ir.bilgisoft.toopeto.entities.Conversation.Smp.STATUS_NONE;
	...
	protected boolean initSmp(final String question, final String secret) {
		...
		if (session!=null) {
			try {
				...
				mConversation.smp().status = ir.bilgisoft.toopeto.entities.Conversation.Smp.STATUS_WE_REQUESTED;
	...
	protected boolean verifyWithUri(ir.bilgisoft.toopeto.utils.XmppUri uri) {
		ir.bilgisoft.toopeto.entities.Contact contact = mConversation.getContact();
	...
	protected boolean handleIntent(Intent intent) {
		if (intent != null && intent.getAction().equals(ACTION_VERIFY_CONTACT)) {
			...
			try {
				this.mConversation = this.xmppConnectionService.find(this.mAccount, Jid.fromString(intent.getExtras().getString("contact")).getLocalpart());
