...
import ir.bilgisoft.toopeto.entities.Message.ImageParams;
import ir.bilgisoft.toopeto.entities.Message;

public class MessageAdapter extends ArrayAdapter<ir.bilgisoft.toopeto.entities.Message> {
	...
	private ir.bilgisoft.toopeto.ui.ConversationActivity activity;
	...
	public MessageAdapter(ir.bilgisoft.toopeto.ui.ConversationActivity activity, List<ir.bilgisoft.toopeto.entities.Message> messages) {
	...
	public int getItemViewType(int position) {
		if (getItem(position).wasMergedIntoPrevious()) {
			return NULL;
		} else if (getItem(position).getType() == ir.bilgisoft.toopeto.entities.Message.TYPE_STATUS) {
			return STATUS;
		} else if (getItem(position).getStatus() <= ir.bilgisoft.toopeto.entities.Message.STATUS_RECEIVED) {
			return RECEIVED;
		} else {
			return SENT;
		}
	...
	private void displayStatus(ViewHolder viewHolder, ir.bilgisoft.toopeto.entities.Message message) {
		...
		boolean multiReceived = message.getConversation().getMode() == ir.bilgisoft.toopeto.entities.Conversation.MODE_MULTI
				&& message.getMergedStatus() <= ir.bilgisoft.toopeto.entities.Message.STATUS_RECEIVED;
		if (message.getType() == ir.bilgisoft.toopeto.entities.Message.TYPE_IMAGE || message.getType() == ir.bilgisoft.toopeto.entities.Message.TYPE_FILE || message.getDownloadable() != null) {
			...
			if (message.getDownloadable() != null && message.getDownloadable().getStatus() == ir.bilgisoft.toopeto.entities.Downloadable.STATUS_FAILED) {
				error = true;
			}
		}
		switch (message.getMergedStatus()) {
		case ir.bilgisoft.toopeto.entities.Message.STATUS_WAITING:
			info = getContext().getString(R.string.waiting);
			break;
		case ir.bilgisoft.toopeto.entities.Message.STATUS_UNSEND:
			ir.bilgisoft.toopeto.entities.Downloadable d = message.getDownloadable();
			if (d!=null) {
			...
			break;
		case ir.bilgisoft.toopeto.entities.Message.STATUS_OFFERED:
			info = getContext().getString(R.string.offering);
			break;
		case ir.bilgisoft.toopeto.entities.Message.STATUS_SEND_RECEIVED:
			if (activity.indicateReceived()) {
				viewHolder.indicatorReceived.setVisibility(View.VISIBLE);
			}
			break;
		case ir.bilgisoft.toopeto.entities.Message.STATUS_SEND_DISPLAYED:
			if (activity.indicateReceived()) {
				viewHolder.indicatorReceived.setVisibility(View.VISIBLE);
			}
			break;
		case ir.bilgisoft.toopeto.entities.Message.STATUS_SEND_FAILED:
			info = getContext().getString(R.string.send_failed);
			error = true;
			break;
		default:
			if (multiReceived) {
				ir.bilgisoft.toopeto.entities.Contact contact = message.getContact();
				if (contact != null) {
		...
		String formatedTime = ir.bilgisoft.toopeto.utils.UIHelper.readableTimeDifferenceFull(getContext(),
	...
	private void displayTextMessage(ViewHolder viewHolder, ir.bilgisoft.toopeto.entities.Message message) {
		if (viewHolder.download_button != null) {
			viewHolder.download_button.setVisibility(View.GONE);
		}
		viewHolder.image.setVisibility(View.GONE);
		...
		if (message.getBody() != null) {
			if (message.getType() != ir.bilgisoft.toopeto.entities.Message.TYPE_PRIVATE) {
	...
	private void displayDownloadableMessage(ViewHolder viewHolder,
			final ir.bilgisoft.toopeto.entities.Message message, String text) {
		viewHolder.image.setVisibility(View.GONE);
	...
	private void displayOpenableMessage(ViewHolder viewHolder,final ir.bilgisoft.toopeto.entities.Message message) {
		final ir.bilgisoft.toopeto.entities.DownloadableFile file = activity.xmppConnectionService.getFileBackend().getFile(message);
		viewHolder.image.setVisibility(View.GONE);
		viewHolder.messageBody.setVisibility(View.GONE);
		viewHolder.download_button.setVisibility(View.VISIBLE);
	...
	private String getDisplayedMucCounterpart(final ir.bilgisoft.toopeto.xmpp.jid.Jid counterpart) {
		if (counterpart==null) {
			return "";
	...
	public View getView(int position, View view, ViewGroup parent) {
		final ir.bilgisoft.toopeto.entities.Message message = getItem(position);
		...
		if (type == STATUS) {
			...
			if (viewHolder.message_box != null) {
				Log.e(ir.bilgisoft.toopeto.Config.LOGTAG, "detected type=NULL but with wrong cached view");
			...
			Log.e(ir.bilgisoft.toopeto.Config.LOGTAG,"detected wasMergedIntoPrevious with wrong type");
			return view;
			...
			} else if (conversation.getMode() == ir.bilgisoft.toopeto.entities.Conversation.MODE_MULTI) {
		...
		if (message.getDownloadable() != null && message.getDownloadable().getStatus() != ir.bilgisoft.toopeto.entities.Downloadable.STATUS_UPLOADING) {
			ir.bilgisoft.toopeto.entities.Downloadable d = message.getDownloadable();
			if (d.getStatus() == ir.bilgisoft.toopeto.entities.Downloadable.STATUS_DOWNLOADING) {
			...
			} else if (d.getStatus() == ir.bilgisoft.toopeto.entities.Downloadable.STATUS_CHECKING) {
				displayInfoMessage(viewHolder,activity.getString(R.string.checking_image));
			} else if (d.getStatus() == ir.bilgisoft.toopeto.entities.Downloadable.STATUS_DELETED) {
				if (message.getType() == ir.bilgisoft.toopeto.entities.Message.TYPE_FILE) {
			...
			} else if (d.getStatus() == ir.bilgisoft.toopeto.entities.Downloadable.STATUS_OFFER) {
				if (message.getType() == ir.bilgisoft.toopeto.entities.Message.TYPE_FILE) {
			...
			} else if (d.getStatus() == ir.bilgisoft.toopeto.entities.Downloadable.STATUS_OFFER_CHECK_FILESIZE) {
				displayDownloadableMessage(viewHolder, message,activity.getString(R.string.check_image_filesize));
			} else if (d.getStatus() == ir.bilgisoft.toopeto.entities.Downloadable.STATUS_FAILED) {
				if (message.getType() == ir.bilgisoft.toopeto.entities.Message.TYPE_FILE) {
		...
		} else if (message.getType() == ir.bilgisoft.toopeto.entities.Message.TYPE_IMAGE && message.getEncryption() != ir.bilgisoft.toopeto.entities.Message.ENCRYPTION_PGP && message.getEncryption() != ir.bilgisoft.toopeto.entities.Message.ENCRYPTION_DECRYPTION_FAILED) {
			displayImageMessage(viewHolder, message);
		} else if (message.getType() == ir.bilgisoft.toopeto.entities.Message.TYPE_FILE && message.getEncryption() != ir.bilgisoft.toopeto.entities.Message.ENCRYPTION_PGP && message.getEncryption() != ir.bilgisoft.toopeto.entities.Message.ENCRYPTION_DECRYPTION_FAILED) {
	...
	public void startDownloadable(ir.bilgisoft.toopeto.entities.Message message) {
	...
	public void openDownloadable(ir.bilgisoft.toopeto.entities.DownloadableFile file) {
		if (!file.exists()) {
