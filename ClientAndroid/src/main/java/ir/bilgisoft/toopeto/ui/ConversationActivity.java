...
import ir.bilgisoft.toopeto.services.XmppConnectionService.OnConversationUpdate;
import ir.bilgisoft.toopeto.services.XmppConnectionService.OnRosterUpdate;
...
public class ConversationActivity extends XmppActivity
	implements OnAccountUpdate, OnConversationUpdate, OnRosterUpdate, ir.bilgisoft.toopeto.xmpp.OnUpdateBlocklist {
	...
	private List<ir.bilgisoft.toopeto.entities.Conversation> conversationList = new ArrayList<>();
	private ir.bilgisoft.toopeto.entities.Conversation mSelectedConversation = null;
	private ListView listView;
	private ConversationFragment mConversationFragment;

	private ArrayAdapter<ir.bilgisoft.toopeto.entities.Conversation> listAdapter;//b
...
public void updateConversationList()
{
    ...
    public List<ir.bilgisoft.toopeto.entities.Conversation> getConversationList() {
	...
	public ir.bilgisoft.toopeto.entities.Conversation getSelectedConversation() {
		return this.mSelectedConversation;
	}

	public void setSelectedConversation(ir.bilgisoft.toopeto.entities.Conversation conversation) {
	...
	protected String getShareableUri() {
		ir.bilgisoft.toopeto.entities.Conversation conversation = getSelectedConversation();
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        ///conversationList=countryArray.;
        listView = (ListView) findViewById(R.id.list);
		this.listAdapter = new ir.bilgisoft.toopeto.ui.adapter.ConversationAdapter(this, conversationList);//b*
   //     ArrayAdapter adapter = new ArrayAdapter<String>(this,
	...
	@Override
	public void switchToConversation(ir.bilgisoft.toopeto.entities.Conversation conversation) {
	...
	private void updateActionBarTitle(boolean titleShouldBeName) {
		...
		final ir.bilgisoft.toopeto.entities.Conversation conversation = getSelectedConversation();
		if (ab != null) {
			if (titleShouldBeName && conversation != null) {
				ab.setDisplayHomeAsUpEnabled(true);
				ab.setHomeButtonEnabled(true);
				if (conversation.getMode() == ir.bilgisoft.toopeto.entities.Conversation.MODE_SINGLE || useSubjectToIdentifyConference()) {
	...
	public void sendReadMarkerIfNecessary(final ir.bilgisoft.toopeto.entities.Conversation conversation) {
		...
		if (isConversationsOverviewVisable()
			...
			if (this.getSelectedConversation() != null) {
				if (this.getSelectedConversation().getLatestMessage()
						.getEncryption() != ir.bilgisoft.toopeto.entities.Message.ENCRYPTION_NONE) {
					menuSecure.setIcon(R.drawable.ic_action_secure);
						}
				if (this.getSelectedConversation().getMode() == ir.bilgisoft.toopeto.entities.Conversation.MODE_MULTI) {
					menuContactDetails.setVisible(false);
					menuAttach.setVisible(false);
					menuBlock.setVisible(false);
					menuUnblock.setVisible(false);
	...
	public void verifyOtrSessionDialog(final ir.bilgisoft.toopeto.entities.Conversation conversation, View view) {
		...
		popup.setOnMenuItemClickListener(new OnMenuItemClickListener() {
			...
			public boolean onMenuItemClick(MenuItem menuItem) {
				Intent intent = new Intent(ConversationActivity.this, ir.bilgisoft.toopeto.ui.VerifyOTRActivity.class);
				intent.setAction(ir.bilgisoft.toopeto.ui.VerifyOTRActivity.ACTION_VERIFY_CONTACT);
				intent.putExtra("contact", conversation.getContact().getJid().toBareJid().toString());
				intent.putExtra("account", conversation.getAccount().getJid().toBareJid().toString());
				switch (menuItem.getItemId()) {
					case R.id.scan_fingerprint:
						intent.putExtra("mode", ir.bilgisoft.toopeto.ui.VerifyOTRActivity.MODE_SCAN_FINGERPRINT);
						break;
					case R.id.ask_question:
						intent.putExtra("mode", ir.bilgisoft.toopeto.ui.VerifyOTRActivity.MODE_ASK_QUESTION);
						break;
					case R.id.manual_verification:
						intent.putExtra("mode", ir.bilgisoft.toopeto.ui.VerifyOTRActivity.MODE_MANUAL_VERIFICATION);
						break;
				}
				startActivity(intent);
				return true;
	...
	protected void selectEncryptionDialog(final ir.bilgisoft.toopeto.entities.Conversation conversation) {
		...
		if (fragment != null) {
				...
				public boolean onMenuItemClick(MenuItem item) {
					switch (item.getItemId()) {
						case R.id.encryption_choice_none:
							conversation.setNextEncryption(ir.bilgisoft.toopeto.entities.Message.ENCRYPTION_NONE);
							item.setChecked(true);
							break;
						case R.id.encryption_choice_otr:
							conversation.setNextEncryption(ir.bilgisoft.toopeto.entities.Message.ENCRYPTION_OTR);
							item.setChecked(true);
							break;
						case R.id.encryption_choice_pgp:
							if (hasPgp()) {
								if (conversation.getAccount().getKeys()
										.has("pgp_signature")) {
									conversation
										.setNextEncryption(ir.bilgisoft.toopeto.entities.Message.ENCRYPTION_PGP);
									item.setChecked(true);
								} else {
			...
			switch (conversation.getNextEncryption(forceEncryption())) {
				case ir.bilgisoft.toopeto.entities.Message.ENCRYPTION_NONE:
					none.setChecked(true);
					break;
				case ir.bilgisoft.toopeto.entities.Message.ENCRYPTION_OTR:
					otr.setChecked(true);
					break;
				case ir.bilgisoft.toopeto.entities.Message.ENCRYPTION_PGP:
	...
	protected void muteConversationDialog(final ir.bilgisoft.toopeto.entities.Conversation conversation) {
		AlertDialog.Builder builder = new AlertDialog.Builder(this);
	...
	public void unmuteConversation(final ir.bilgisoft.toopeto.entities.Conversation conversation) {
	...
	void onBackendConnected() {
		...
		if (xmppConnectionService.getAccounts().size() == 0) {
			startActivity(new Intent(this, ir.bilgisoft.toopeto.ui.EditAccountActivity.class));
		...
		ir.bilgisoft.toopeto.utils.ExceptionHelper.checkForCrash(this, this.xmppConnectionService);
	...
	private void selectConversationByUuid(String uuid) {
		for (ir.bilgisoft.toopeto.entities.Conversation aConversationList : conversationList) {
	...
	private void attachFileToConversation(ir.bilgisoft.toopeto.entities.Conversation conversation, Uri uri) {
		prepareFileToast = Toast.makeText(getApplicationContext(),
				getText(R.string.preparing_file), Toast.LENGTH_LONG);
		prepareFileToast.show();
		xmppConnectionService.attachFileToConversation(conversation,uri, new UiCallback<ir.bilgisoft.toopeto.entities.Message>() {
			@Override
			...
			@Override
			public void error(int errorCode, ir.bilgisoft.toopeto.entities.Message message) {
	...
	private void attachImageToConversation(ir.bilgisoft.toopeto.entities.Conversation conversation, Uri uri) {
		prepareFileToast = Toast.makeText(getApplicationContext(),
				getText(R.string.preparing_image), Toast.LENGTH_LONG);
		prepareFileToast.show();
		xmppConnectionService.attachImageToConversation(conversation, uri,
				new UiCallback<ir.bilgisoft.toopeto.entities.Message>() {

					@Override
					public void userInputRequried(PendingIntent pi,
							ir.bilgisoft.toopeto.entities.Message object) {
						hidePrepareFileToast();
					}
	...
	public void encryptTextMessage(ir.bilgisoft.toopeto.entities.Message message) {
		xmppConnectionService.getPgpEngine().encrypt(message,
				new UiCallback<ir.bilgisoft.toopeto.entities.Message>() {

					@Override
					public void userInputRequried(PendingIntent pi,
							ir.bilgisoft.toopeto.entities.Message message) {
						ConversationActivity.this.runIntent(pi,
	...
	public void unblockConversation(final ir.bilgisoft.toopeto.entities.Blockable conversation) {
		xmppConnectionService.sendUnblockRequest(conversation);
	}

	public void blockConversation(final ir.bilgisoft.toopeto.entities.Blockable conversation) {
