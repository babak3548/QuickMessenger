...
import ir.bilgisoft.toopeto.entities.ListItem;
...
public class ListItemAdapter extends ArrayAdapter<ListItem> {
	...
	public View getView(int position, View view, ViewGroup parent) {
		...
		List<ir.bilgisoft.toopeto.entities.ListItem.Tag> tags = item.getTags();
		if (tags.size() == 0 || !this.showDynamicTags) {
			tagLayout.setVisibility(View.GONE);
		} else {
			tagLayout.setVisibility(View.VISIBLE);
			tagLayout.removeAllViewsInLayout();
			for(ir.bilgisoft.toopeto.entities.ListItem.Tag tag : tags) {
		...
		final ir.bilgisoft.toopeto.xmpp.jid.Jid jid = item.getJid();
		if (jid != null) {
